...
import daoEmployees.CustomerDAO;
import daoEmployees.EmployeeDAO;
import daoEmployees.OrderDAO;
...
import modelEmployees.Customer;
import modelEmployees.Employee;
...
public class EmployeeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (path.endsWith("/employee/loginEmployees")) {
            request.getRequestDispatcher(folder + "/loginEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/orderListEmployees")) {
            ResultSet rs = ordDAO.getOrderByEmployeePhone(phone);
            request.setAttribute("rs", rs);
            request.getRequestDispatcher(folder + "/orderListEmployees.jsp").forward(request, response);
        } else if (path.startsWith("/employee/customerInfoEmployees")) {
            ...
            request.getRequestDispatcher(folder + "/customerInfoEmployees.jsp").forward(request, response);
        } else if (path.startsWith("/employee/searchByOrderIdEmployees")) {
            try {
                ...
                while (rs.next()) {
                    ...
                    searchResultHtml += "<td><a class=\"btn btn-outline-primary\" id=\"btnView\" href=\"/orderDetailEmployees/" + rs.getString("ord_id") + "\" style=\"font-weight: bold; font-size: 25px\">View</a></td>";
                    searchResultHtml += "</tr>";
                }
        ...
        } else if (path.endsWith("/employee/logoutEmployees")) {
            session.removeAttribute("phone");
            session.removeAttribute("fullname");
            session.invalidate();
            request.getRequestDispatcher(folder + "/loginEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/loginEmployees/fail")) {
            String loginError = "The phone or password is incorrect!";
            request.setAttribute("loginError", loginError);
            request.getRequestDispatcher(folder + "/loginEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/updateProfileEmployees")) {
            request.getRequestDispatcher(folder + "/updateProfileEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/updateProfileSuccessEmployees")) {
            ...
            request.getRequestDispatcher(folder + "/updateProfileEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/newOrderListEmployees")) {
            request.getRequestDispatcher(folder + "/newOrderListEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/changePasswordEmployees")) {
            request.getRequestDispatcher(folder + "/changePasswordEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/changePasswordSuccessEmployees")) {
            request.setAttribute("result", "Password is changed successfully!");
            request.getRequestDispatcher(folder + "/changePasswordEmployees.jsp").forward(request, response);
        } else if (path.endsWith("/employee/changePasswordFailEmployees")) {
            request.setAttribute("result", "Old password is incorrect! Please Try Again!");
            request.getRequestDispatcher(folder + "/changePasswordEmployees.jsp").forward(request, response);
        } else if (path.startsWith("/employee/takeOrderEmployees/")) {
            String[] data = path.split("/");
            String orderId = data[data.length - 1];
            ordDAO.setPhoneForOrder(phone, orderId);
            response.sendRedirect("/employee/orderListEmployees");
        } else if (path.endsWith("/employee/forgotPassEmployees")) {
            request.getRequestDispatcher(folder + "/enterEmailForgotPassEmployees.jsp").forward(request, response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (btnLogin != null && btnLogin.equals("login")) {
            ...
            if (emp != null) {
                ...
                response.sendRedirect("/employee/loginEmployees/fail");
            }
        }
        ...
        if (btnUpdateStatus != null && btnUpdateStatus.equals("Update")) {
            ...
            if (kq != 0) {
                response.sendRedirect("/orderDetailEmployees/updateSuccess/" + orderId);
            } else {
                response.sendRedirect("/orderDetailEmployees/updateFail/" + orderId);
            }
        }
        if (btnUpdateProfile != null && btnUpdateProfile.equals("Update")) {
            ...
            if (kq != 0) {
                ...
                response.sendRedirect("/employee/updateProfileSuccessEmployees");
            } else {
                response.sendRedirect("/employee/updateProfileFailEmployees");
            }
        }
        if (btnChangePassword != null && btnChangePassword.equals("ChangePassword")) {
            ...
            if (correctOldPass != null) {
                ...
                response.sendRedirect("/employee/changePasswordSuccessEmployees");
            } else {
                response.sendRedirect("/employee/changePasswordFailEmployees");
            }
        }
    ...
    private boolean sendEmail(String to, String subject, String body) {
        ...
        String port = "587";
        String username = "perfumestoremanager607@gmail.com";
